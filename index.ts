...
const init_controller = () => {
	...
	size_controller();
	buttons_add(new Button(controller.enter.dst as Rectangle, undefined, undefined, () => {console.log("enter"); get_context().ws.send('select')}))
	buttons_add(new Button(controller.exit.dst as Rectangle, undefined, undefined, () => {console.log("exit"); get_context().ws.send('back')}))
	buttons_add(new Button(controller.up.dst as Rectangle, undefined, undefined, () => {console.log("up"); get_context().ws.send('up')}))
	buttons_add(new Button(controller.down.dst as Rectangle, undefined, undefined, () => {console.log("down"); get_context().ws.send('down')}))
	buttons_add(new Button(controller.left.dst as Rectangle, undefined, undefined, () => {console.log("left"); get_context().ws.send('left')}))
	buttons_add(new Button(controller.right.dst as Rectangle, undefined, undefined, () => {console.log("right"); get_context().ws.send('right')}))
}
...
const size_portrait = () => {
	const dims = get_context().dimensions;
	const pad = 25;
	const enter_origin:Point  = {y: dims.y * 0.20 , x:dims.x * 0.5 - dims.x * 0.125};
	const exit_origin:Point  = {y: dims.y - dims.y * 0.35 , x:dims.x * 0.5 - dims.x * 0.25 };
